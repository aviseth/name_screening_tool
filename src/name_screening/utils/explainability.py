...
from ..models.matching import MatchConfidenceLevel, MatchExplanation, MatchingLayer
...
class ExplainabilityGenerator:
    ...
    def _generate_reasoning(
        ...
        formatters = {
            MatchingLayer.EXACT: lambda: template.format(
                input=input_name, found=found_name
            ),
        ...
        return (
            formatter()
            if formatter
            else f"Match between '{input_name}' and '{found_name}'"
        )
    ...
    def _explain_structural_match(self, context: Optional[Dict[str, Any]]) -> str:
        ...
        return (
            ", ".join(explanations)
            if explanations
            else "structural similarity detected"
        )
    ...
    def _extract_evidence(
        ...
        evidence_map = {
            MatchingLayer.NICKNAME: lambda: [f"Source: {context['nickname_source']}"]
            if "nickname_source" in context
            else [],
            MatchingLayer.STRUCTURAL: lambda: [
                item
                for item in [
                    f"Matching parts: {', '.join(context['matching_components'])}"
                    if "matching_components" in context
                    else None,
                    f"Pattern: {context['pattern']}" if "pattern" in context else None,
                ]
                if item
            ],
            MatchingLayer.FUZZY: lambda: [
                item
                for item in [
                    f"Similarity: {context['similarity_score']:.1%}"
                    if "similarity_score" in context
                    else None,
                    f"Edit distance: {context['edit_distance']}"
                    if "edit_distance" in context
                    else None,
                ]
                if item
            ],
            MatchingLayer.LLM_ENHANCED: lambda: [
                item
                for item in [
                    f"Cultural factors: {', '.join(context['cultural_factors'])}"
                    if context.get("cultural_factors")
                    else None,
                    f"Context clues: {', '.join(context['context_clues'])}"
                    if context.get("context_clues")
                    else None,
                    f"Role: {context['contextual_role']['role_description']}"
                    if context.get("contextual_role", {}).get("role_description")
                    else None,
                    f" {context['confidence_adjustment']}"
